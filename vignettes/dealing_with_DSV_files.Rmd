...
should be represented in plain text, but a lot of possible customization are left to
the final users, it is very common to incurr in issues importing data from this kind 
of files. 
The purpose of this article is to show some problems you can encounter 
importing data in a SQLite database from DSV files and how to deal with them.
...
Let's start with a simple example using real world data. To retrieve the data
we need, we can use the package `package?piggyback` to download some public data that 
has been stored in the [RSQLite.toolkit-tests](https://github.com/fab-algo/RSQLite.toolkit-tests) GitHub repository[^1].
...
comma to separate fields' values and the dot as a decimal separator),
...
We can check the structure of the dataset using the `file_schema_dsv()` function,
...
(i.e. `header = TRUE, sep = ",", dec = "."`), we can call that function using the 
`input_file` parameter alone.
...
Now we connect to a sample database (i.e. `tests.sqlite`) and move the
...
The warning message tells us that something probably went wrong. If we check the 
...
creating the file we can follow those guidelines, otherwise we have to inspect the file
and determine them by direct guess.

To explore a DSV file (especially big files) it is better to use log file explorers
and avoid general purpose text editors. As an example, we can use [klogg](https://klogg.filimonov.dev/) that is available for all major operating systems.
...
- many strings are using special characters that need a specific enconding 
  system; in this file the UTF-8 enconding is used.

For the input file used in this example we need:

- to explicitly force the `quote` parameter to `"\""` (i.e. only the double quote character)
  and the `na.strings` parameter to `""`;
- to turn off the commenting option: `comment.char = ""`;
- to tell to the `base::scan()` function that the input file is encoded in UTF-8: 
  `fileEncoding = "UTF-8"`.

We also need to delete the data from the first import attempt and load it from scratch:
`drop_table = TRUE`.
