...
Let's start with a simple example using real world data. To retrieve some examples
data, we can use the package [piggyback](https://github.com/ropensci/piggyback) to 
download some public datasets that has been stored in the 
...
and then it calls the *`scan`* function to actually read the file content in a
data frame and write it to the SQLite table.
...
column names when importing data from DSV files. It does so throught the
...
data_file <- file.path(tempdir(),
                 "Blockchain/Blockchain_Banking_Scopus_Dataset_2015_2025.csv")
```
...
As we can see there are some problematic column names, like for example `Author(s) ID`,
...
To handle in a safer way DSV files (and avoid some unexpected results) when
importing data in a SQLite database using the `dbTableFromDSV()` function, you
should always:

- inspect the input file to understand how data is represented there &rarr; to that
  end you can use the `file_schema_dsv()` function to help you in this task and test
  different parameters settings before importing the data;
...
- check the column names used in the input file and choose the right strategy
  to handle them &rarr; here the `id_quote_method` parameter of the `dbTableFromDSV()`
