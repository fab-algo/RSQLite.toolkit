...
should be represented in plain text, but a lot of possible customizations are left to
...
we need, we can use the package `package?piggyback` to download some public datasets 
that has been stored in the [RSQLite.toolkit-tests](https://github.com/fab-algo/RSQLite.toolkit-tests) GitHub repository[^1].
...
(i.e. `header = TRUE, sep = ",", dec = ".", grp = ""`), we can call that function
using the `input_file` parameter alone.
...
We also need to delete the data from the first import attempt and load it from 
scratch: `drop_table = TRUE`.
