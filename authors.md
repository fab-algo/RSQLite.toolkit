# Authors and Citation

## Authors

- **Ludovico G. Beretta**. Author, maintainer, copyright holder.

## Citation

Source:
[`DESCRIPTION`](https://github.com/fab-algo/RSQLite.toolkit/blob/v0.1.0.9010/DESCRIPTION)

Beretta L (2026). *RSQLite.toolkit: Load Data in SQLite from Tabular
Files*. R package version 0.1.0.9010,
<https://github.com/fab-algo/RSQLite.toolkit>.

    @Manual{,
      title = {RSQLite.toolkit: Load Data in SQLite from Tabular Files},
      author = {Ludovico G. Beretta},
      year = {2026},
      note = {R package version 0.1.0.9010},
      url = {https://github.com/fab-algo/RSQLite.toolkit},
    }
